...
export function useKubeObject<T extends KubeObjectClass>({
  ...
  function startWatching() {
    ...
    socketRef.current = watchObject({
      endpoint,
      name,
      onObject: obj => client.setQueryData<KubeObjectInterface>(queryKey, obj),
  ...
  return { ...query, data: query.error ? null : query.data ?? null };
}
...
export function useKubeObjectList<T extends KubeObjectClass>({
  ...
  return {
    ...query,
    items: query.error ? null : query.data?.items ?? null,
    data: query.error ? null : query.data ?? null,
  };
}
